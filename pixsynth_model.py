...
class PixSynthModel():
    def __init__(self):
        ...
        # 初始创建PixSynth模型实例
        self.pixsynth_model = self.vgg_layers(self.style_layers + self.content_layers)
        self.pixsynth_model.trainable = False
    ...
    def style_conver_model(self, input_img):
        ...
        pixsynth = self.pixsynth_model

        inputs = input_img * 255.0
        preprocessed_input = tf.keras.applications.vgg19.preprocess_input(inputs)
        outputs = pixsynth(preprocessed_input)
...
if __name__ == '__main__':
    ...
    # 加载图片
    content_image = pixsynthModel.load_image(content_image_url, is_local=False)
    style_image = pixsynthModel.load_image(style_image_url, is_local=True)
    pixsynthModel.imshow(content_image)
    pixsynthModel.imshow(style_image)

    # 风格转换并保存
    content_is_local = False
    style_is_local = True
    save_path = 'result/pixsynth_model/dog_style02_img.png'
    pixsynthModel.style_conver(save_path, content_image_url, style_image_url, content_is_local, style_is_local)
